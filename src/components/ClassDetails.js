...
class ClassDetails extends Component{
	...
	render(){
		return(
			this.props.classes.length ? this.props.classes.map((classRoom, i) => {
				return(
					this.props.match.params.id == classRoom.id ? (
						<div key={i}>
							...
							<div className='classroom-container'>
								<div className='classroom-header-div'>
									...
									<i className={this.state.expand ? "fas fa-sort-up expand-arrow": "fas fa-sort-down expand-arrow"} onClick={this.expandHandler}></i>
								</div>
								{this.state.expand ? (
									...
									<div className='classroom-post-section'>
										{!this.state.post ? (
														...
														return (
															<div key={i}>
																<div className='post-actions' style={{display: "flex", alignItems: "center"}}>
																	{!post.edit ? (<p style={{margin: "0 10px 0 0"}}>{post.value}</p>): <input type="text" value={post.value} onChange={this.editPost.bind(this, i)}/> }
																	{!post.edit ? (<i className="fas fa-pencil-alt" onClick={this.postActionHandler.bind(this, "edit", i)}></i>):(<i className="fas fa-check" onClick={this.confirmInput.bind(this, i)}></i>)}
																	<i style={{marginLeft: "10px"}} className="fas fa-trash-alt" onClick={this.postActionHandler.bind(this, "delete", i)}></i>
																</div>
															</div>
														)
													})
											...
											<div className='post-container'>
												<textarea value={this.state.postInput} placeholder="enter some text" onChange={this.postInputHandler}></textarea>
